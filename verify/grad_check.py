...
loose = np.array([grad_full_sz(r.t, r.v, r.sv, r.a, r.w-r.sw/2, r.w+r.sw/2, tol=1e-12)[0][0] for r in g.itertuples()])
tight = np.array([grad_full_sz(r.t, r.v, r.sv, r.a, r.w-r.sw/2, r.w+r.sw/2, tol=1e-40)[0][0] for r in g.itertuples()])
...
try:
    ...
    kg = np.array([K.g_full_sz(np.array([r.t]), r.v, r.sv, r.a, r.w-r.sw/2, r.w+r.sw/2)[0] for r in g.itertuples()])
    print(f"compiled kernel vs numpy, 400 sets: max abs {np.abs(kg - loose).max():.1e}")
    assert np.abs(kg - loose).max() < 1e-13
    f = pd.read_csv("data/wienr_full.csv")
...
for n in (8, 16, 24, 32):
    e_plain = max(abs(rule(n, 1, r) - ref[i]) / ref[i] for i, r in enumerate(f.itertuples()) if big[i])
    e_map = max(abs(rule(n, 3, r) - ref[i]) / ref[i] for i, r in enumerate(f.itertuples()) if big[i])
    print(f"  {n:2d} nodes: plain Gauss-Legendre {e_plain:.1e}   with cubic edge map {e_map:.1e}")
e32 = max(abs(rule(32, 3, r) - ref[i]) / ref[i] for i, r in enumerate(f.itertuples()) if big[i])
assert e32 < 1e-8
