...
Let the starting point be uniformly distributed on $[w_1, w_2]$ with $0 < w_1 \le w_2 < 1$. In practice the endpoints are excluded because at $w_1 = 0$ the mixed density behaves like $u^{-1/2}$ as $u \to 0$. Indeed, the quadrature over the non-decision-time window used in existing implementations does not generally cover that singularity with software either rejecting the input or returning a zero density. The density~\eqref{eq:result} itself is defined there. The lower-barrier first-passage time subdensity of the resulting process is
\begin{equation}
...
\end{align}
Fast and accurate algorithms for $W(z)$ are standard \citep{ZaghloulAli2011} and available in scientific libraries.  Equation~\eqref{eq:faddeeva} must be evaluated through $W$. Written with the imaginary error function, it loses about $\pi^2 k^2/(4\kappa)$ in the exponent to cancellation. Furthermore in our implementation we switch from the small- to the large-time form at $t/a^2 = 1$. While the choice is not critical, since the two forms agree to thirteen relative digits for $0.1 \le t/a^2 \le 1$ and to eleven at $t/a^2 = 2$. 

\section{Convergence}
\label{sec:conv}

\begin{sloppypar}
Both series are infinite and are truncated after $J$ terms for~\eqref{eq:result} and $K$ terms for~\eqref{eq:result_large}. The $j$-th term of~\eqref{eq:result} is at most $c\,(j+1)a\exp(-j^2 a^2/(2t))$ in absolute value, and the $k$-th term of~\eqref{eq:result_large} is at most $c'\,k\exp(-k^2\pi^2 t/(2a^2))$, where $c$ and $c'$ do not depend on the index (\ref{app:bound}). From $J = 2$ on, these bounds shrink faster than a geometric series, so the truncation error is at most a constant multiple of the first omitted term. The constants $c$ and $c'$ carry all of the dependence on $\nu$ and $\eta$ but are the same for every term, so they cancel when the error is measured relative to the leading term. The rule is therefore a relative-error criterion, like those of \citet{NavarroFuss2009} and \citet{Gondan2014}. It makes the exponential factor of the first omitted term smaller than a tolerance $\varepsilon$ and adds one term to cover the remaining factors. This last step is not proven, and we check it numerically below. The numbers of terms are:
\end{sloppypar}
\begin{equation}
...
Section~\ref{sec:verify} checks the result on a fixed lattice of parameter values. We consider two further grids test what that lattice does not reach: the values that published fits of the model actually take, and the limit in which the starting-point range closes to a point.
...
The supplementary material contains (i) an array implementation of~\eqref{eq:result}, \eqref{eq:result_large}, \eqref{eq:seven}, and all partial derivatives in Python, together with the compiled implementation used for Table~\ref{tab:timing}; (ii) the symbolic verification script; (iii) the scripts and reference values for the comparisons with WienR, rtdists, and HDDM, for both grids of \ref{app:grids} and (iv) the Lean formalization.
